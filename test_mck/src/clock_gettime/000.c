...
RUN_FUNC(TEST_SUITE, TEST_NUMBER)
{
	...
	printf("[INFO] Spending cpu time, please wait...\n");
	...
	for (i = 0; i < sizeof(ctc)/sizeof(ctc[0]); ++i)
	{
		...
		int result = clock_gettime(ctc[i].clock_id, &ts);
		tp_assert(result == ctc[i].expect_result, "clock_gettime return unexpected result.");
		if(result == 0) {
			printf("%-20s: %ld.%09ld\n", ctc[i].clock_name, ts.tv_sec, ts.tv_nsec);
		} else {
			printf("%-20s: %s %s\n", ctc[i].clock_name, strerror(errno),
				(errno == ctc[i].expect_errno) ? "(expected)":"(non expected)");
			tp_assert(errno == ctc[i].expect_errno, "clock_gettime return unexpected errno.");
		}
	}
	return NULL;
}
