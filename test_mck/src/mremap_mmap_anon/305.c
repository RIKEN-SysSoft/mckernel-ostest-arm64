/* 305.c COPYRIGHT FUJITSU LIMITED 2015-2016 */
#include "test_mck.h"
#include "testsuite.h"
#include <signal.h>
...
RUN_FUNC(TEST_SUITE, TEST_NUMBER)
{
	struct mremap_mmap_anon_param params[] = {
		{.size = MREMAP_RANGE_LARGE_SIZE, .nopage = 0, .prot = PROT_READ|PROT_WRITE, .shared = 1}, // old > new(move)
	...
	{//signal
		const char* msg;
		char ch;
		msg = mremap_mmap_anon_setup_signal_handler(SIGBUS);
		tp_assert(msg == NULL, msg);
		ch = *(remap + old_size);
		UNUSED_VARIABLE(ch);
	}
