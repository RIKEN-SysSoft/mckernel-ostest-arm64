/* 207.c COPYRIGHT FUJITSU LIMITED 2015 */
#include "test_mck.h"
#include "testsuite.h"
...
RUN_FUNC(TEST_SUITE, TEST_NUMBER)
{
	struct mremap_mmap_anon_param params[] = {
		{.size = MREMAP_RANGE_LARGE_SIZE, .nopage = 0, .prot = PROT_READ|PROT_WRITE},    // old > free
		{.size = MREMAP_RANGE_LARGE_SIZE, .nopage = 0, .prot = PROT_NONE},               // use   use
		{.size = MREMAP_RANGE_LARGE_SIZE*2, .nopage = 0, .prot = PROT_READ|PROT_WRITE},  // use   new/use
		{.size = MREMAP_RANGE_LARGE_SIZE, .nopage = 0, .prot = PROT_NONE},               // use   use
	};
	...
	{//verify
		...
		msg = mremap_mmap_anon_data_check(params[2].addr + new_size,
						  params[2].size - new_size, '2');
		tp_assert(msg == NULL, msg);
	}
