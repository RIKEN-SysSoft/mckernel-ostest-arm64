/* 019.c COPYRIGHT FUJITSU LIMITED 2018 */
#include "test_mck.h"
#include "testsuite.h"
#include <stdio.h>
#include <sys/types.h>
...
RUN_FUNC(TEST_SUITE, TEST_NUMBER)
{
	...
	/* all PAGE setting MADV_DONTDUMP. */
	ret = madvise((char *)buf, 3 * PAGE_SIZE, MADV_DONTDUMP);
	if (ret != 0) {
		printf("madvise failed (errno=%d)\n", errno);
		return "NG.";
	}

	/* center PAGE setting MADV_DODUMP. */
	ret = madvise((char *)buf + PAGE_SIZE, PAGE_SIZE, MADV_DODUMP);
	if (ret != 0) {
		printf("madvise failed (errno=%d)\n", errno);
		return "NG.";
	}
