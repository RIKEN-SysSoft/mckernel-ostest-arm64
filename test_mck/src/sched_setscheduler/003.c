...
RUN_FUNC(TEST_SUITE, TEST_NUMBER)
{
	...
	int status = 0;
	int policy = 0;
	struct sched_param set_param = { 0 };
	struct sched_param get_param = { 0 };

	status = sched_setscheduler(pid, SCHED_OTHER, &set_param);
	printf("sched_setscheduler(%d, SCHED_OTHER, %p) = %d, errno = %d\n", pid, &set_param, status, errno);
	tp_assert(status != -1, "sched_setscheduler() failed.");

	policy = sched_getscheduler(pid);
	printf("sched_getscheduler(%d) = %d, errno = %d\n", pid, policy, errno);
	tp_assert(policy == SCHED_OTHER, "sched_getscheduler() failed.");
