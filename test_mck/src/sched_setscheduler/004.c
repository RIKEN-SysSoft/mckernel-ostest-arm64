...
RUN_FUNC(TEST_SUITE, TEST_NUMBER)
{
	...
	status = sched_setscheduler(pid, SCHED_BATCH, &set_param);
	printf("sched_setscheduler(%d, SCHED_BATCH, %p) = %d, errno = %d\n", pid, &set_param, status, errno);
	tp_assert(status != -1, "sched_setscheduler() failed.");

	policy = sched_getscheduler(pid);
	printf("sched_getscheduler(%d) = %d, errno = %d\n", pid, policy, errno);
	tp_assert(policy == SCHED_BATCH, "sched_getscheduler() failed.");
