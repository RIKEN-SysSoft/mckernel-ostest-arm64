...
RUN_FUNC(TEST_SUITE, TEST_NUMBER)
{
	...
	l_ret = mbind(buf, PAGE_SIZE, MPOL_BIND, &mbind_node_mask, BITS_PER_LONG, 0);
	tp_assert(l_ret==0, "mbind(mode=MPOL_BIND) error");

	/* get_mempolicy and tp_assert(get_mempolicy) */
	i_ret = -1;
	i_ret = get_mempolicy(&mode, node_mask, max_node, buf, MPOL_F_ADDR);
	tp_assert(i_ret==0, "get_mempolicy1(flags=MPOL_F_ADDR) error");
	tp_assert(mode==MPOL_BIND, "mode != MPOL_BIND");
	tp_assert(node_mask[0]==mbind_node_mask, "node_mask != mbind_node_mask");
	node_mask[0] = 0;

	/* mbind and tp_assert(mbind) */
	l_ret = -1;
	l_ret = mbind(buf, PAGE_SIZE, MPOL_DEFAULT, NULL, 0, 0);
	tp_assert(l_ret==0, "mbind(mode=MPOL_DEFAULT) error");
	...
	node_mask[0] = 0x01;
	i_ret = get_mempolicy(&mode, node_mask, max_node, buf, MPOL_F_ADDR);
	tp_assert(i_ret==0, "get_mempolicy2(flags=MPOL_F_ADDR) error");
