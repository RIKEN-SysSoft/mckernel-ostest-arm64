...
RUN_FUNC(TEST_SUITE, TEST_NUMBER)
{
	int mode = -1, i_ret = -1;
	long l_ret = -1;
	unsigned long mbind_node_mask = 0x0F;
	...
	l_ret = mbind(buf, PAGE_SIZE, MPOL_BIND, &mbind_node_mask, 2, 0);
	...
	printf("mbind_node_mask = %lu\nnode_mask = %lu\n", mbind_node_mask, node_mask[0]);
	tp_assert(node_mask[0]==0x01, "node_mask != 0x03");
