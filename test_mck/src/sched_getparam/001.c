...
RUN_FUNC(TEST_SUITE, TEST_NUMBER)
{
	pid_t pid = getpid();
	int status = 0;
	struct sched_param dummy_param = { 50 };
	struct sched_param set_param = { 80 };
	struct sched_param get_param = { 0 };

	status = sched_getparam(0, &get_param);
	printf("sched_getparam(%d, %p) = %d, errno = %d\n", 0, &get_param, status, errno);
	tp_assert(status != -1, "sched_getparam() failed.");
	tp_assert(get_param.sched_priority == 0, "sched_getparam() failed.");

	status = sched_setscheduler(pid, SCHED_RR, &dummy_param);
	...
	printf("sched_getparam(%d, %p) = %d, errno = %d\n", 0, &get_param, status, errno);
	tp_assert(status != -1, "sched_getparam() failed.");
	tp_assert(get_param.sched_priority == dummy_param.sched_priority, "sched_getparam() failed.");
